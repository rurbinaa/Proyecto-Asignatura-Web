...
export default function DateRangePicker({ startDate, endDate, onChange, minDate, maxDate, size = 'medium' }) {
  ...
  const [viewYear, setViewYear] = useState(() => {
    const now = new Date();
    return endDate ? new Date(endDate).getFullYear() : now.getFullYear();
  });
  const [viewMonth, setViewMonth] = useState(() => {
    const now = new Date();
    return endDate ? new Date(endDate).getMonth() : now.getMonth();
  });
  ...
  const handleDayClick = (day) => {
    ...
    if (selecting === 'start') {
      onChange({ startDate: dateStr, endDate: endDate && dateStr > endDate ? '' : endDate });
      setSelecting('end');
    } else {
